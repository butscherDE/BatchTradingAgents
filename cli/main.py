...
def etrade_auth(
    consumer_key: str = typer.Option(
        ..., "--key", "-k",
        help="E*TRADE consumer key (oauth_consumer_key)",
    ),
    consumer_secret: str = typer.Option(
        ..., "--secret", "-s",
        help="E*TRADE consumer secret",
    ),
    sandbox: bool = typer.Option(
        True, "--sandbox/--live",
        help="Use sandbox (paper) or production (live) environment",
    ),
    list_accounts: bool = typer.Option(
        False, "--list-accounts", "-l",
        help="After auth, list all accounts to find account_id_key",
    ),
):
    """Authorize with E*TRADE OAuth and get access tokens for config."""
    from cli.etrade_client import authorize_etrade, list_etrade_accounts as _list_accounts

    console.print(Panel(
        "[bold]E*TRADE OAuth Authorization[/bold]\n\n"
        f"Environment: {'[yellow]SANDBOX[/yellow]' if sandbox else '[red]PRODUCTION[/red]'}\n"
        "This will open a URL for you to authorize the app.",
        title="E*TRADE Auth",
    ))

    try:
        token, token_secret = authorize_etrade(
            consumer_key, consumer_secret, is_paper=sandbox
        )
    except Exception as e:
        console.print(f"[red]Authorization failed: {e}[/red]")
        raise typer.Exit(1)

    console.print("\n[green bold]Add these to your account config in application.conf:[/green bold]\n")
    console.print(f'  api_key = "{consumer_key}"')
    console.print(f'  api_secret = "{consumer_secret}"')
    console.print(f'  oauth_token = "{token}"')
    console.print(f'  oauth_token_secret = "{token_secret}"')

    if list_accounts:
        console.print("\n[cyan]Fetching accounts...[/cyan]")
        try:
            accounts = _list_accounts(
                consumer_key, consumer_secret, token, token_secret, is_paper=sandbox
            )
            table = Table(title="E*TRADE Accounts")
            table.add_column("account_id_key", style="green")
            ...
            for acct in accounts:
                table.add_row(
                    acct["account_id_key"],
                    acct["account_id"],
                    acct["account_name"] or acct["account_desc"],
                    acct["account_type"],
                )
            console.print(table)
            console.print("\n[dim]Use the account_id_key value for etrade_account_id_key in config[/dim]")
        except Exception as e:
            console.print(f"[red]Failed to list accounts: {e}[/red]")
