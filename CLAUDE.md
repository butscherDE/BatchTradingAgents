...
The continuous evaluation service runs on a Windows machine at `10.0.0.216`.

**SSH access:**
```
ssh -i ~/.ssh/key_butschpc danie@10.0.0.216
```
...
- Full Swagger docs at `http://10.0.0.216:8000/docs`

**Run remote commands via SSH:**
```
ssh -i ~/.ssh/key_butschpc danie@10.0.0.216 "command here"
```
...
```bash
# Check service logs
curl -s "http://10.0.0.216:8000/api/logs?limit=20" | python3 -m json.tool

# Check task stats
curl -s "http://10.0.0.216:8000/api/tasks/stats" | python3 -m json.tool

# Pull latest code on server
ssh -i ~/.ssh/key_butschpc danie@10.0.0.216 "cd C:\Users\danie\repos\BatchTradingAgents && git pull"

# Restart service (stop + start)
ssh -i ~/.ssh/key_butschpc danie@10.0.0.216 "taskkill /F /IM python.exe & cd C:\Users\danie\repos\BatchTradingAgents && start /B python -m service.main"

# Backfill news from yfinance (pulls latest news for all watchlist tickers into DB)
ssh -i ~/.ssh/key_butschpc danie@10.0.0.216 "cd C:\Users\danie\repos\BatchTradingAgents && python -m service.tools backfill"

# Backfill with date filter (only news after a specific date)
ssh -i ~/.ssh/key_butschpc danie@10.0.0.216 "cd C:\Users\danie\repos\BatchTradingAgents && python -m service.tools backfill --since 2026-05-07"

# Replay articles through GPU pipeline (resubmits queued articles to GPU worker)
# Requires service to be running
ssh -i ~/.ssh/key_butschpc danie@10.0.0.216 "cd C:\Users\danie\repos\BatchTradingAgents && python -m service.tools replay --since 2026-05-08T00:00:00"

# Delete DB and start fresh (stop service first)
ssh -i ~/.ssh/key_butschpc danie@10.0.0.216 "cd C:\Users\danie\repos\BatchTradingAgents && del data\service.db"
```
